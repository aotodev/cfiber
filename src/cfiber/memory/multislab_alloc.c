...
static void* default_alloc(const size_t size, void* ctx) {
    (void)ctx;
    return aligned_alloc(CFIBER_CACHE_LINE_SIZE, align_up(size, CFIBER_CACHE_LINE_SIZE));
}
...
static void list_push(cfiber_slab_node_t** head, cfiber_slab_node_t* const node) {
    node->next = *head;
    node->prev = nullptr;
    if (*head) {
        (*head)->prev = node;
...
static void list_unlink(cfiber_slab_node_t** head, cfiber_slab_node_t* const node) {
    if (node->prev) {
...
static cfiber_slab_node_t* multislab_grow(cfiber_multislab_t* ms) {
    if (ms->max_slabs && ms->slab_count >= ms->max_slabs) {
        return nullptr;
    }

    cfiber_slab_node_t* const node = ms->mem_alloc(sizeof(cfiber_slab_node_t), ms->mem_ctx);
    if (UNLIKELY(!node)) {
        return nullptr;
    }
    ...
    if (UNLIKELY(!mem)) {
        ms->mem_free(node, sizeof(cfiber_slab_node_t), ms->mem_ctx);
        return nullptr;
    }

    const int res = cfiber_slab_init(&node->slab, ms->block_size, mem, ms->slab_memory_size);
    if (UNLIKELY(res)) {
...
static void multislab_free_node(cfiber_multislab_t* ms, cfiber_slab_node_t* const node) {
    ms->mem_free(node->raw_memory, ms->slab_memory_size, ms->mem_ctx);
    ms->mem_free(node, sizeof(cfiber_slab_node_t), ms->mem_ctx);
}

static cfiber_slab_node_t* find_owning_slab(const cfiber_multislab_t* const ms, const void* const ptr) {
    const uintptr_t p = (uintptr_t)ptr;

    /* check active list */
    for (cfiber_slab_node_t* n = ms->active; n; n = n->next) {
    ...
    /* check full list */
    for (cfiber_slab_node_t* n = ms->full; n; n = n->next) {
        const uintptr_t base = (uintptr_t)n->raw_memory;
        if (p >= base && p < base + ms->slab_memory_size) {
            return n;
        }
...
int cfiber_multislab_init_ext(cfiber_multislab_t* const ms,
                              ...
                              void* mem_ctx) {
    if (block_size < CFIBER_CACHE_LINE_SIZE || (block_size % CFIBER_CACHE_LINE_SIZE) || blocks_per_slab == 0
        || blocks_per_slab > CFIBER_SLAB_MAX_BLOCKS || blocks_per_slab > SIZE_MAX / block_size || !mem_alloc
        || !mem_free) {
        return -1;
    }
...
int cfiber_multislab_init(cfiber_multislab_t* ms,
                          const size_t block_size,
                          const uint32_t blocks_per_slab,
                          const uint32_t max_slabs,
                          const uint32_t hysteresis_threshold) {
    return cfiber_multislab_init_ext(
        ms, block_size, blocks_per_slab, max_slabs, hysteresis_threshold, default_alloc, default_free, nullptr);
}

void* cfiber_multislab_alloc(cfiber_multislab_t* ms) {
    ...
    void* ptr = nullptr;
    while (ms->active) {
        ptr = cfiber_slab_alloc(&ms->active->slab);
        if (LIKELY(ptr)) {
            break;
        }
        cfiber_slab_node_t* const full = ms->active;
    ...
    cfiber_slab_node_t* const node = ms->active;
...
void cfiber_multislab_release(cfiber_multislab_t* const ms, void* const ptr) {
    cfiber_slab_node_t* node = find_owning_slab(ms, ptr);
    if (UNLIKELY(!node)) {
    ...
    if (UNLIKELY(!cfiber_slab_release(&node->slab, ptr))) {
        return;
    }
    node->used_count--;

    /* Tracked membership, not inferred from used_count: a full slab can still
     * be on the active list (see cfiber_multislab_alloc). */
...
static void free_node_list(cfiber_multislab_t* ms, cfiber_slab_node_t* head) {
    while (head) {
        cfiber_slab_node_t* next = head->next;
        multislab_free_node(ms, head);
        head = next;
    }
}

void cfiber_multislab_destroy(cfiber_multislab_t* ms) {
