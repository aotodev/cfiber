 ...
 *
 * blocks_per_slab ranges past CFIBER_BITMAP_WORD_BITS so the multi-word bitmap scan
...
#define LIVE_CAP 512
#define PER_SLAB_MAX (3 * CFIBER_BITMAP_WORD_BITS)
...
static void check_list(const cfiber_slab_node_t* n, bool full, uint32_t per_slab, uint32_t cap, list_stats* st) {
...
static bool owned_by_some_slab(const cfiber_multislab_t* ms, const void* p) {
    const cfiber_slab_node_t* lists[2] = {ms->active, ms->full};
    for (size_t li = 0; li < 2; li++) {
        for (const cfiber_slab_node_t* n = lists[li]; n; n = n->next) {
...
static void check_invariants(const cfiber_multislab_t* ms, void* const* live, size_t n) {
    list_stats st = {0};
...
int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    const size_t block_size = (size_t)CFIBER_CACHE_LINE_SIZE * fuzz_range(&in, 1, 4);
    ...
    if (cfiber_multislab_init_ext(
            &ms, block_size, per_slab, max_slabs, hysteresis, counting_alloc, counting_free, &ctx)) {
        return 0;
    }
    ...
    while (fuzz_remaining(&in) > 0) {
        ...
        if ((op & 1u) == 0u) {
            ...
            if (n > 0) {
                ...
                cfiber_multislab_release(&ms, live[idx]);
                live[idx] = live[--n];
            }
        }
    ...
    for (size_t i = 0; i < n; i++) {
        cfiber_multislab_release(&ms, live[i]);
    }
